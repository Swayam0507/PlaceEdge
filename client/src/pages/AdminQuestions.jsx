        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="flex items-center gap-2 font-display font-bold text-2xl text-ink mb-1">
              <FileText className="text-emerald" /> Manage Questions
            </h1>
        ...
        <div className="bg-paper p-4 rounded-2xl border border-line shadow-sm flex flex-col md:flex-row items-center gap-4">
          <form onSubmit={handleSearch} className="relative w-full flex-1">
            <input type="text" placeholder="Search questions..." value={filters.search}
              ...
              className="w-full pl-11 pr-4 py-2.5 rounded-xl border border-line bg-surface text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all text-sm" id="admin-question-search" />
          ...
          <select value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value, page: 1 })}
            className="w-full md:w-auto px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all text-sm appearance-none font-medium" id="admin-category-filter">
          ...
          <select value={filters.difficulty} onChange={(e) => setFilters({ ...filters, difficulty: e.target.value, page: 1 })}
            className="w-full md:w-auto px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all text-sm appearance-none font-medium" id="admin-difficulty-filter">
          ...
          <>
                      ...
                      <tr key={q._id} className="hover:bg-surface/50 transition-colors">
                        ...
                        <td className="p-4">
                          <span className={`inline-flex px-2.5 py-1 rounded-md text-xs font-bold capitalize ${
                            ...
                            q.category === 'technical' ? 'bg-teal-100 text-teal-700' : 'bg-line text-ink-soft'
                          }`}>
                            {q.category}
                          </span>
                        </td>
                        ...
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-2">
                            <button className="p-2 text-amber-deep hover:bg-amber/10 rounded-lg transition-colors" onClick={() => openEditModal(q)} title="Edit">
                              <Edit2 size={16} />
                            </button>
            ...
            {pagination.total > 1 && (
              <div className="flex items-center justify-between bg-paper p-4 rounded-2xl border border-line shadow-sm">
                <button disabled={filters.page <= 1} onClick={() => setFilters({ ...filters, page: filters.page - 1 })} className="px-4 py-2 text-sm font-bold bg-surface border border-line rounded-xl text-ink hover:bg-line disabled:opacity-50 transition-colors">← Prev</button>
                ...
                <button disabled={filters.page >= pagination.total} onClick={() => setFilters({ ...filters, page: filters.page + 1 })} className="px-4 py-2 text-sm font-bold bg-surface border border-line rounded-xl text-ink hover:bg-line disabled:opacity-50 transition-colors">Next →</button>
              </div>
            )}
          </>
        )}
            ...
            <div className="bg-paper rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
              <div className="px-6 py-4 border-b border-line flex items-center justify-between bg-surface/50">
                <h2 className="font-display font-bold text-xl text-ink flex items-center gap-2">
                  {editingQuestion ? <><Edit2 className="text-amber-deep" /> Edit Question</> : <><Plus className="text-emerald" /> Add Question</>}
                </h2>
              ...
              <form onSubmit={handleSave} className="p-6 overflow-y-auto space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select id="q-category" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all appearance-none">
                  ...
                  <div>
                    ...
                    <select id="q-difficulty" value={formData.difficulty} onChange={(e) => setFormData({ ...formData, difficulty: e.target.value })} className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all appearance-none">
                      <option value="easy">Easy</option>
                      <option value="medium">Medium</option>
                      <option value="hard">Hard</option>
                    </select>
                ...
                <div>
                  ...
                  <textarea id="q-text" rows="3" value={formData.question} onChange={(e) => setFormData({ ...formData, question: e.target.value })} placeholder="Enter the question..." className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all resize-y" />
                </div>
                <div className="space-y-3 bg-surface/50 p-4 rounded-xl border border-line">
                  ...
                  {formData.options.map((opt, i) => (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <input type="text" value={opt} onChange={(e) => handleOptionChange(i, e.target.value)} placeholder={`Option ${i + 1}`} id={`option-text-${i}`} className={`flex-1 px-4 py-2 rounded-xl border ${formData.correctAnswer === i ? 'border-emerald ring-1 ring-emerald/20 bg-emerald-soft/30' : 'border-line bg-paper'} text-ink focus:outline-none transition-all`} />
                    </div>
                  ))}
                </div>
                <div>
                  <label htmlFor="q-explanation" className="block text-sm font-bold text-ink mb-1.5">Explanation (optional)</label>
                  <textarea id="q-explanation" rows="2" value={formData.explanation} onChange={(e) => setFormData({ ...formData, explanation: e.target.value })} placeholder="Explain the correct answer..." className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all resize-y" />
                </div>
        ...
        {showBulkModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/50 backdrop-blur-sm animate-fade-in" onClick={() => setShowBulkModal(false)}>
            <div className="bg-paper rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
              <div className="px-6 py-4 border-b border-line flex items-center justify-between bg-surface/50">
                <h2 className="font-display font-bold text-xl text-ink flex items-center gap-2">
                  <Upload className="text-amber-deep" /> Bulk Import Questions
                </h2>
              ...
              <div className="p-6 overflow-y-auto space-y-4">
                <p className="text-sm text-muted font-medium">
                  Upload a JSON file or paste JSON array below. Each question must have: <code className="bg-line px-1.5 py-0.5 rounded text-ink font-mono text-xs">category</code>, <code className="bg-line px-1.5 py-0.5 rounded text-ink font-mono text-xs">question</code>, <code className="bg-line px-1.5 py-0.5 rounded text-ink font-mono text-xs">options</code> (array of 4), <code className="bg-line px-1.5 py-0.5 rounded text-ink font-mono text-xs">correctAnswer</code> (0-3), <code className="bg-line px-1.5 py-0.5 rounded text-ink font-mono text-xs">difficulty</code>.
                </p>
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setBulkJSON(sampleJSON)} className="px-4 py-2 bg-paper border border-line rounded-xl text-sm font-bold text-ink hover:bg-surface transition-colors">
                    Load Sample
                  </button>
                </div>
                ...
                <textarea
                  ...
                  rows="12"
                  className="w-full px-4 py-3 rounded-xl border border-line bg-surface text-ink font-mono text-xs focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all resize-y"
                />
