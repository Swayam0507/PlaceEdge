      ...
      <div className="flex flex-col space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="flex items-center gap-2 font-display font-bold text-2xl text-ink mb-1">
              <Users className="text-emerald" /> Manage Students
            </h1>
        ...
        <div className="bg-paper p-4 rounded-2xl border border-line shadow-sm flex items-center justify-between">
          <form onSubmit={handleSearch} className="relative w-full max-w-md">
            <input
              ...
              className="w-full pl-11 pr-4 py-2.5 rounded-xl border border-line bg-surface text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all text-sm"
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-surface/50 border-b border-line text-xs uppercase tracking-wider text-muted font-semibold">
                      <th onClick={() => handleSort("name")} className="p-4 cursor-pointer hover:bg-line/50 transition-colors">
                        <span className="flex items-center gap-1.5">Name <SortIcon field="name" /></span>
                      </th>
                      <th className="p-4">Email</th>
                      <th onClick={() => handleSort("branch")} className="p-4 cursor-pointer hover:bg-line/50 transition-colors">
                        <span className="flex items-center gap-1.5">Branch <SortIcon field="branch" /></span>
                      </th>
                      <th onClick={() => handleSort("cgpa")} className="p-4 cursor-pointer hover:bg-line/50 transition-colors">
                        <span className="flex items-center gap-1.5">CGPA <SortIcon field="cgpa" /></span>
                      </th>
                      <th onClick={() => handleSort("totalTests")} className="p-4 cursor-pointer hover:bg-line/50 transition-colors">
                        <span className="flex items-center gap-1.5">Tests <SortIcon field="totalTests" /></span>
                      </th>
                      <th onClick={() => handleSort("avgScore")} className="p-4 cursor-pointer hover:bg-line/50 transition-colors">
                      ...
                      <tr key={user._id} className="hover:bg-surface/50 transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-amber/20 text-amber-deep flex items-center justify-center font-bold text-xs shrink-0">
                        ...
                        <td className="p-4">
                          <div className="flex items-center justify-center gap-2">
                            <button className="p-2 text-emerald hover:bg-emerald/10 rounded-lg transition-colors" onClick={() => openDetailModal(user)} title="View Details">
                              <Eye size={16} />
                            </button>
                            <button className="p-2 text-amber-deep hover:bg-amber/10 rounded-lg transition-colors" onClick={() => openEditModal(user)} title="Edit">
                              <Edit2 size={16} />
                            </button>
        ...
        {editingUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/50 backdrop-blur-sm animate-fade-in" onClick={() => setEditingUser(null)}>
            <div className="bg-paper rounded-2xl shadow-xl w-full max-w-md overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="px-6 py-4 border-b border-line flex items-center justify-between bg-surface/50">
                <h2 className="font-display font-bold text-xl text-ink flex items-center gap-2">
                  <Edit2 className="text-amber-deep" /> Edit Student
                </h2>
                <button className="p-2 text-muted hover:text-ink hover:bg-line rounded-xl transition-colors" onClick={() => setEditingUser(null)}>
                  <X size={20} />
                </button>
              </div>
              <form onSubmit={handleUpdate} className="p-6 space-y-4">
                <div>
                  ...
                  <input id="edit-name" type="text" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} placeholder="Student Name" required className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all" />
                </div>
                <div>
                  <label htmlFor="edit-email" className="block text-sm font-bold text-ink mb-1.5">Email</label>
                  <input id="edit-email" type="email" value={editForm.email} onChange={(e) => setEditForm({ ...editForm, email: e.target.value })} placeholder="Student Email" required className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="edit-branch" className="block text-sm font-bold text-ink mb-1.5">Branch</label>
                    <input id="edit-branch" type="text" value={editForm.branch} onChange={(e) => setEditForm({ ...editForm, branch: e.target.value })} placeholder="e.g., Computer Science" className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all" />
                  </div>
                  <div>
                    <label htmlFor="edit-semester" className="block text-sm font-bold text-ink mb-1.5">Semester</label>
                    <input id="edit-semester" type="number" min="1" max="8" value={editForm.semester} onChange={(e) => setEditForm({ ...editForm, semester: parseInt(e.target.value) || 1 })} className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="edit-cgpa" className="block text-sm font-bold text-ink mb-1.5">CGPA</label>
                    <input id="edit-cgpa" type="number" step="0.01" min="0" max="10" value={editForm.cgpa} onChange={(e) => setEditForm({ ...editForm, cgpa: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all" />
                  </div>
                  <div>
                    <label htmlFor="edit-role" className="block text-sm font-bold text-ink mb-1.5">Role</label>
                    <select id="edit-role" value={editForm.role} onChange={(e) => setEditForm({ ...editForm, role: e.target.value })} className="w-full px-4 py-2.5 rounded-xl border border-line bg-paper text-ink focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-all">
                ...
                <div className="pt-4 flex items-center justify-end gap-3 border-t border-line mt-6">
                  <button type="button" className="px-5 py-2.5 text-sm font-bold text-ink bg-line hover:bg-line rounded-xl transition-colors" onClick={() => setEditingUser(null)}>Cancel</button>
        ...
        {detailUser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/50 backdrop-blur-sm animate-fade-in" onClick={() => setDetailUser(null)}>
            <div className="bg-paper rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
              <div className="px-6 py-4 border-b border-line flex items-center justify-between bg-surface/50">
                <h2 className="font-display font-bold text-xl text-ink flex items-center gap-2">
                  <Eye className="text-amber-deep" /> Student Profile
                </h2>
                <button className="p-2 text-muted hover:text-ink hover:bg-line rounded-xl transition-colors" onClick={() => setDetailUser(null)}>
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto">
                {/* Student Info */}
                <div className="flex items-center gap-4 p-4 bg-amber/10/50 rounded-xl border border-indigo-100 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-deep/80 font-medium">{detailUser.email}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                  {[
                    ...
                    <div key={i} className="p-3 bg-paper border border-line rounded-xl text-center shadow-sm">
                ...
                {detailLoading ? (
                  ...
                  <div className="mb-6">
                    <h4 className="flex items-center gap-2 font-bold text-sm text-ink mb-3 uppercase tracking-wider">
                      <TrendingUp className="text-amber-deep" /> Score Trend
                    </h4>
                    <div className="h-48 p-4 bg-paper border border-line rounded-xl shadow-sm">
                  ...
                  <p className="text-center text-muted py-8 text-sm font-medium bg-surface rounded-xl mb-6">No test history available.</p>
                )}
                ...
                {detailTests.length > 0 && (
                  <div>
                    <h4 className="flex items-center gap-2 font-bold text-sm text-ink mb-3 uppercase tracking-wider">
                      <Calendar className="text-amber-deep" /> Recent Tests
                    </h4>
                    <div className="border border-line rounded-xl overflow-hidden shadow-sm">
                      <table className="w-full text-left text-sm">
                        <thead className="bg-surface/80 border-b border-line text-xs uppercase tracking-wider text-muted font-semibold">
                          <tr>
